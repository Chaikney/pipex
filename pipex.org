...
* memory issues
It is very hard to find out if there is anything going on with allocations using valgrind when there are subprocesses.
#+begin_quote https://softwareengineering.stackexchange.com/questions/140483/is-it-a-waste-of-time-to-free-resources-before-i-exit-a-process
all memory allocated to a process gets released when the process is terminated, so if the only requirement is to release the allocated memory, doing it explicitly is indeed a waste of time.
#+end_quote
That might mean that I don't have to worry so much about this issue for this project but it does feel very icky.
src_sh{valgrind -s --show-leak-kinds=all --leak-check=full --track-origins=yes --track-fds=yes ./$(NAME)}
* Cleanly ending
The subject allows the use of exit, perror and strerror but i don't know best practice for finishing that all off. If I perror before exit, i guess something gets output before the end of the program? Could I use strerror to  make a more useful error code, or use a different error number that is more useful than just EXIT_FAILURE?
...
We copy STDIN or OUT to the read / write end of our pipe using dup2
...
NOTE that argv[0] is the name of the file being called, which is very handy because no changes are needed in the find path, send args approach I have.
#+begin_quote
...
*** man 3 exit
src_c{void exit (int status);}
status is EXIT_SUCCESS or EXIT_FAILURE.
Simple, just ends the program and returnss the status. Doesn't help with error messages much (AFAICT).
** open files
Fairly simple operations, use open to get the fd for the file. Careful with the options.
*** 1 for reading
src_c{in_file = open(argv[1], O_RDONLY);}
...
*** 2 for writing
this one especially there are more possiblities. WRITE? APPEND? Note that there are different definitions for what should happen with  the files in the normal and bonus / here_doc cases.
src_c{out_file = open(argv[4], O_CREAT);}
** launch a new command
*** fork...
Make a child process to run the first command.
...
[[file:~/code/pipex/pipex.c::// TODO Should have some sensible limits on the file names-
// ....What could cause bother?]]
